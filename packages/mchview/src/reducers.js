...
export const selectors = {
  ...
  isRightPanelVisible: (state) => visibilitySelectors.isRightPanelVisible(state.visibility),
  currentElement: (state) => viewSelectors.currentElement(state.view),
  area: (state) => state.area,
  dsValue: (state, dsid) => dataSelectors.dsValue(state.data, dsid),
  isVisible: (state, category) => outlineSelectors.isVisible(state.outline, category),
  outlineStyle: (state, category) => outlineSelectors.style(state.outline, category),
  isFetching: (state) => selectors.isFetchingDePlane(state, selectors.dePlaneId(state))
    || selectors.isFetchingDualSampas(state),
  isFetchingDualSampas: (state) => envelopSelectors.isFetchingDualSampas(
    state.envelop,
    selectors.dePlaneId(state),
  ),
  isFetchingDe: () => false,
  isFetchingDePlane: (state, id) => envelopSelectors.isFetchingDePlane(state.envelop, id),
  degeo: (state) => envelopSelectors.deplane(state.envelop, selectors.dePlaneId(state)),
  dualsampas: (state) => envelopSelectors.dualsampas(state.envelop, selectors.dePlaneId(state)),
